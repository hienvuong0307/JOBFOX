    ...
    <div>
        <div class="flex flex-col">
            ...
            <div class="absolute top-0 w-full flex justify-center z-10 mt-[15rem]">
                <img style="height: 300px; width: 4000px" src="@/views/pages/SVG/banner10.svg" alt="Banner" />
            </div>
        </div>
    </div>
      <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
          <span class="text-3xl text-white">Create a Job Post</span>
      </div>

      <div class="absolute top-[75%] left-1/2 transform -translate-x-1/2 text-center z-10">
          <Breadcrumb style="background-color: transparent" :model="items">
              <template #item="{ item }">
                  <a class="!cursor-pointer" href=""
                      ><span class="text-slate-400 hover:text-white text-2xl">{{ item.label }}</span></a
                  >
              </template>
          </Breadcrumb>
      </div>
